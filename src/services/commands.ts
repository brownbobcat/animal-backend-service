...
async function handleAnimalCreate(args: string[]): Promise<void> {
  if (args.length < 2) {
  ...
  const animalData = args[1] as string;
  ...
  if (newAnimal) {
    ...
    console.log(
      "Failed to create animal. Please check your data format and try again."
    );
  }
}

async function userCommands(authToken: string | undefined): Promise<void> {
  if (authToken) {
